...
function SignIn() {
  ...
  const token = useSelector((state) => state.auth.token); // Token aus Redux holen

  const onSubmit = async (data) => {
    try {
      const response = await fetch("/api/auth/signin", {
        // Nutzt die Next.js API-Route, statt direkt ans express-backend zu senden
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(data), // Sende die Login-Daten mit
      });

      const result = await response.json(); // Backend-Antwort holen/ auslesen
      console.log(result);

      if (!response.ok) {
        throw new Error(result.message || "Invalid credentials"); // Hier die echte Servermeldung nutzen
      }

      alert("Login successful!");

      // Redux-Token speichern
      dispatch(setToken(result.token));

      router.push("/dashboard");
    } catch (error) {
      console.error("Login error:", error.message); // Fehler mit richtiger Message anzeigen
      alert(error.message); // Dem User die korrekte Meldung zeigen
    }
  };
